...
// Layer 5 - Pi Agent. Held via WINFN + "/" (MO(_AGENT) lives in the Win Fn
// layer). Sits between CURSOR(4) and MENU(6) in keymap.c's layer_names enum
// -- MENU is bumped to 6 explicitly so this slot stays free without needing
// its own enum entry (mirrors how the Kick75 keymap this was ported from
// just uses a bare #define rather than folding it into an enum).
#define _AGENT 5

// Custom keycode base. Chains after this keymap's own custom_keycodes enum
// (MENU_CW/MENU_CCW in keymap.c, which itself starts at SAFE_RANGE) rather
// than starting at SAFE_RANGE directly, since both enums share the same
// keycode space.
#define AG_KC_BASE (MENU_CCW + 1)
...
// own cheat sheet while WINFN+"/" is held.
//
static inline void agent_set_color_scaled(uint8_t led, uint8_t r, uint8_t g, uint8_t b) {
    // Unlike the Kick75 this was ported from, AK820 Pro's RGB Matrix driver
    // doesn't swap the R/G channels, so no GRB workaround is needed here. If
    // colors ever come out swapped on real hardware, swap the g/r arguments
    // in the rgb_matrix_set_color() call below.
...
static inline void agent_layer_leds(uint8_t led_min, uint8_t led_max) {
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
        for (uint8_t col = 0; col < MATRIX_COLS; col++) {
            ...
            agent_set_color_scaled(led, c[0], c[1], c[2]);
        }
    }
}
